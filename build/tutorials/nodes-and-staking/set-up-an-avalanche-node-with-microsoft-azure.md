...
＃使用Microsoft Azure运行雪崩节点

运行验证器并与Avalanche赌注可提供9.69％至11.54％之间的极具竞争力的奖励，具体取决于您所投注的时长。最高赌注是通过下注一年来赚取的，而最低赌注是14天。也不用大刀阔斧，因此您无需担心客户端中的硬件故障或错误，而这会导致您失去部分或全部股份。取而代之的是，使用雪崩，您目前仅需要维持至少60％的正常运行时间即可获得奖励。如果您不满足此要求，您将不会受到大刀阔斧的砍伐，但您不会获得任何回报。**您也不需要将私钥放在节点上即可开始在该节点上进行验证。** 即使有人闯入您的云环境并获得对该节点的访问权限，他们所能做的最坏的事情就是关闭该节点。

运行验证程序节点不仅使您能够在AVAX中接收奖励，而且以后您还可以验证生态系统中的其他子网，并在其子网的本机令牌中接收奖励。

您只需要2个CPU内核，4 GB内存和40 GB SSD的适度硬件要求即可运行验证器，它不会消耗大量能量。Avalanche的[革命性共识机制]（https://medium.com/ava-hub/avalanche-consensus-the-biggest-breakthrough-since-nakamoto-66e9917fd656）能够扩展到数以百万计的验证者，同时参与共识无与伦比的权力下放。

当前，成为验证者所需的最低押金为2,000 AVAX \（随着价格的上涨，该金额会随着时间的流逝而减少\）。或者，验证人也可以收取少量费用，以使用户可以将股份下放给他们，以帮助降低运营成本。您可以使用计算器[在这里]（https://vscout.io/）来查看运行节点与委派相比将获得多少奖励。

我鼓励所有人在可能的情况下运行自己的验证器，但对于那些不满足最低抵押要求并希望委托的人，我目前正在运行一个可以在[此处]找到的节点（https://avascan.info/staking / validator / NodeID-MGrikMRTmooL1j7uawPHjaMS1cXkbewdb）。
...
``文本
SCP -i C：\雪崩\ avalanche_key.pem -r Ubuntu的@ EnterYourAzureIPHere：/home/ubuntu/.avalanchego/staking C：\雪崩''``


和以前一样，我们需要用检索到的适当值替换“ EnterYourAzureIPHere”。这会将我们的抵押密钥和抵押证书备份到我们之前创建的C：\ Avalanche文件夹中。

！[发布图片]（https://miro.medium.com/max/358/1*nqsjJAv2fkcLKPri5idN-Q.png）

<!--stackedit_data:
eyJoaXN0b3J5IjpbLTk4MTkxNjkwOF19
-->